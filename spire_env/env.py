    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            if 'start' in cmds:
                ...
                self.conn.log("Start Ironclad...")
                self.conn.send_command("start ironclad"); last_action_time = time.time(); continue

            nav = None
            prio = ['confirm', 'proceed', 'return', 'leave', 'click', 'skip']
            ...
            if nav:
                ...
                self.conn.log(f"Reset清理: {nav}")
                self.conn.send_command(nav); last_action_time = time.time(); continue
